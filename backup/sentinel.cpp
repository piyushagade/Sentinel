#include "platform.h"
    ...
    */
    ...
    // Turn on primary by default in a second
    ...
    if (DIAGNOSTIC && SENTINENCE_ENABLED) {
        ...
        sendresponse(9999);
        sendresponse(millis() / 1000 - sentinence_start_timestamp);
        sendresponse(INTERVALBASE);
        sendresponse(INTERVALMULTIPLIER);
        sendresponse(INTERVALBASE * INTERVALMULTIPLIER);
    ...
    if (
            SENTINENCE_ENABLED && 
            (millis() / 1000 - sentinence_start_timestamp > INTERVALBASE * INTERVALMULTIPLIER)
        ) {
